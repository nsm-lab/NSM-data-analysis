# file generated from go.mod using vgo2nix (https://github.com/adisbladis/vgo2nix)
[
  {
    goPackagePath = "git.torproject.org/user/phw/zoossh.git";
    fetch = {
    type = "git";
    rev = "fd8f41dc08b02ca6e19a823c7ef8ef8985ab377b";
    url = "https://github.com/NullHypothesis/zoossh";
    sha256 = "0wr8z2xk72ai2j9rrvy9c651ld7ygyyyma26wvp4iisb5ms5js4a";
    };
  }

  {
    goPackagePath = "github.com/DataWraith/vptree";
    fetch = {
    type = "git";
    rev = "393576533be9ccfe76091b70a3fab83f4723015c";
    url = "https://github.com/DataWraith/vptree/";
    sha256 = "13h7xfqfa72a25y3d2c52gw2kb2dh8c03pkgqlnx03aw1392m4id";
    };
  }

  {
    goPackagePath = "github.com/NullHypothesis/mlgo";
    fetch = {
    type = "git";
    rev = "5f4671392f0a77ffe2dd06944b913aeabe74883a";
    url = "https://github.com/NullHypothesis/mlgo";
    sha256 = "0kfa63kzq8dj7k571dxcyljm1yzvr9qzvq3qz7gp43kj40yl44hs";
    };
  }

  {
    goPackagePath = "github.com/arbovm/levenshtein";
    fetch = {
    type = "git";
    rev = "48b4e1c0c4d0b8b1864f1bd2cd31bb20147e4636";
    url = "https://github.com/arbovm/levenshtein";
    sha256 = "0nmx2iip8xpnbmy6gvqpc9ikizr33dr40xgv746h0b0by8n7rv7y";
    };
  }

  {
    goPackagePath = "github.com/mcgrew/gostats";
    fetch = {
    type = "git";
    rev = "591ec42eaec3e712415fe2f3d24d97acf183a812";
    url = "https://github.com/mcgrew/gostats";
    sha256 = "0k9xgaygbl9pnvlkkipazky87jmrfrb9310hjz7vxqmjp8yi53pm";
    };
  }
]
